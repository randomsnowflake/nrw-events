...
- Use exact offline fixtures and focused tests while editing: `bash scripts/test.sh --agent tests.test_report`. Run `bash scripts/test.sh --agent` before publication; website release `prepare`/`resume` owns this final suite when deploying both repos. Optional coverage uses `NRW_EVENTS_COVERAGE=1`.
