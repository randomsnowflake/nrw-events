...
| `NRW_EVENTS_BONN_DE_DELAY_SECONDS` | `1.0` | Mindestabstand zwischen serialisierten Requests an `bonn.de`; transiente MyraCDN/Backend-Fehler werden zusätzlich mit Backoff wiederholt. |
| `NRW_EVENTS_GATED_HOST_DELAY_SECONDS` | `1.0` | Mindestabstand zwischen serialisierten Requests je kdvz-Portal hinter dem Botgate (Brühl, Wesseling, Frechen, Hürth, Erftstadt, Zülpich); `bonn.de` nutzt `NRW_EVENTS_BONN_DE_DELAY_SECONDS`. |
| `NRW_EVENTS_GATED_RESPONSE_TTL_HOURS` | `20` | Tages-Cache für erfolgreiche Antworten der kdvz-Portale (`botgate.py`). kdvz sendet `no-store` ohne ETag/Last-Modified, daher wird jede URL höchstens einmal pro Tag gelesen; Wiederholungen und manuelle Läufe am selben Tag lesen lokal. Das Botgate-Cookie wird unter `NRW_EVENTS_CACHE_DIR` bis kurz vor Ablauf wiederverwendet und höchstens einmal pro Prozess neu gelöst. `0` deaktiviert den Antwort-Cache. |
| `NRW_EVENTS_BONN_CALENDAR_LISTINGS` | `0` | `1` crawlt zusätzlich die ~50 paginierten Bonn.de-Kalender- und Kostenlos-Listen. Standardmäßig aus: der citykey-JSON-Feed (ein Request) lieferte am 24.09.2026 4513 von 4534 Terminen, bonn.de steht hinter dem kdvz-Botgate. |
...
| `NRW_EVENTS_DETAIL_CACHE_TTL_HOURS` | `1440` | Maximales Alter gecachter HTML-Detailseiten (60 Tage). Detailseiten werden einmal geladen und wiederverwendet, solange Events sie referenzieren; Datum, Zeit und Wegfall kommen aus Listen, Feeds und APIs, die bei jedem Import frisch geladen werden. Späte Änderungen auf einer Detailseite werden in Kauf genommen. Bonn.de cacht fehlgeschlagene Versuche negativ als leeren Body. `0` deaktiviert Speicher- und Platten-Cache. |
| `NRW_EVENTS_DETAIL_CACHE_IDLE_HOURS` | `336` | Detailseiten, die 14 Tage von keinem Event mehr abgerufen wurden, fallen aus dem Cache. Zugriffe werden höchstens täglich persistiert. |
| `NRW_EVENTS_DETAIL_NEGATIVE_CACHE_HOURS` | `24` | Lebensdauer leerer Platzhalter, die Quellen mit `cache_failures=True` (z. B. Bonn.de) nach einem fehlgeschlagenen Detailabruf speichern. Das Auslaufen des eigenen Request- oder Quellbudgets wird nie als Fehler gecacht. |
| `NRW_EVENTS_DETAIL_FAILURE_CACHE_HOURS` | `168` | Dauerhafte 4xx-Antworten (400/401/403/404/405/406/410/451) einer Detailseite werden eine Woche gemerkt und nicht erneut angefragt. Nach drei 401/403/429-Antworten eines Hosts stellt ein Lauf keine weiteren Detail-Requests an diesen Host. |
| `NRW_EVENTS_DETAIL_ENRICHMENT` | `1` | Gemeinsame, gecachte Anreicherung aus primären Detailseiten; `0` deaktiviert sie. |
| `NRW_EVENTS_DETAIL_BATCH_TIMEOUT_SECONDS` | `45`, Köln Open Data: `240`, Bonn.de-Adapter: kein separates Standardbudget | Gesamtbudget für optionale Detailseiten-Abrufe je Quelle; ein expliziter Wert begrenzt auch den Bonn.de-Adapter und überschreibt das Köln-Budget. Bonn.de liest standardmäßig alle benötigten eindeutigen Detail-URLs innerhalb der Request- und Quellenlimits. Nach Ablauf bleiben die vorhandenen Listendaten erhalten. |
...
| `NRW_EVENTS_ICAL_PRUNE` | `1` | Frühe Auswertung der unveränderten Qualitätsregeln für iCal. Verworfene Kandidaten benötigen keine vollständige Aufbereitung. `0` aktiviert den bisherigen Pfad. Alte Termine werden nicht pauschal entfernt. |
