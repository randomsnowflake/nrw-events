...
_GLUED_SERIES_PREFIX = re.compile(r"^(WORTREICH|LESEZIRKEL|LESUNG UND PERFORMANCE|CUT-UP)(?=[A-ZÄÖÜ])")
_SOFT_HYPHEN = "­"
...
def fetch_literaturhaus() -> list:
    ...
    try:
        events = common.fetch_ical(
            ...
            source,
            "Bonn",
            "lesung literatur autor buchvorstellung gespräch",
            1.0,
            "literaturhaus-bonn",
        )
...
_PARK_TITLE = re.compile(r'mkdf-event-title">\s*<a href="([^"]+)"[^>]*>\s*(.*?)\s*</a>', re.S)
...
def events_from_parkbuchhandlung_html(html: str, detail_fetcher=None) -> list:
    ...
    for card in re.split(_PARK_CARD, html or "")[1:]:
        ...
        event = common.make_event(
            ...
            or common.factual_event_description(
                title,
        ...
        if event:
            ...
            key = event.get("category_key") if event.get("category_key") in {"concert", "stage"} else "talk"
