...
def _detail_description(url: str) -> str:
    try:
        return _meta_description(common.fetch_detail_url(url, cache_namespace="junges-theater-bonn", timeout=15))
    ...
    for index, start_pos in enumerate(starts):
        ...
        for column_index, column in enumerate(columns):
            ...
            for item in items:
                ...
                description = (
                    detail_fetcher(link) if link and not is_kulturgarten and common.window_contains(start_dt) else ""
                )
                if not description:
