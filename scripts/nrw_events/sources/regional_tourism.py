...
def _events_from_shapehub(
    html: str, source: str, base: str, listing_url: str, default_city: str, category: str, trust: float
) -> list:
    events = []
    card_re = re.compile(r'<a href="(?P<href>[^"]+)" class="shapehub-card-link">(?P<body>.*?)</a>', re.S | re.I)
    for m in card_re.finditer(html):
        ...
        text = rc.clean(body)
        location = re.search(
            r"shapehub-location-line.*?<span>(.*?)</span>",
            body,
            re.S | re.I,
        )
...
def _fetch_linz() -> list:
    ...
    try:
        for _ in range(6):
            ...
            dates = [
                common.parse_iso_date(value) for value in re.findall(r"/events/(20\d{2}-\d{2}-\d{2})-", html, re.I)
            ]
            if dates and max(date for date in dates if date) >= common.END_DATE:
                break
            next_page = re.search(r'<li class="next"><a[^>]+href="([^"]+)"', html, re.S | re.I)
...
def _linz_detail_context(html: str) -> dict:
    description = re.search(r'<h1>.*?</h1>\s*<span class="centered">(.*?)</span>', html or "", re.S | re.I)
    venue = re.search(r'<i class="[^"]*\bicon-pin\b[^"]*"></i>\s*([^<]+)', html or "", re.S | re.I)
    event_time = re.search(r'class="[^"]*\bevent-time\b[^"]*".*?(\d{1,2}:\d{2})', html or "", re.S | re.I)
    return {
        "description": common.concise_description(rc.clean(description.group(1) if description else "")),
...
def _linz_fallback_description(title: str, date: str, time_text: str, venue: str, listing_copy: str) -> str:
...
def _events_from_linz(html: str, detail_fetcher=None) -> list:
    ...
    for block in re.split(r'(?=<div class="standardteaser">)', html or "", flags=re.I):
        ...
        title = re.search(r'<div class="h3">\s*<a[^>]*>(.*?)</a>', block, re.S | re.I)
        if not (href and title):
            continue
        link = rc.abs_url("https://www.linz.de", href.group("href"))
        listing_copy_match = re.search(r'<div class="teasertext">(.*?)</div>', block, re.S | re.I)
...
def _events_from_bad_muenstereifel(html: str) -> list:
    ...
    for block in re.findall(
        r'<div class="veranst_singleItem clearfix">(.*?)</div>\s*</div>\s*</div>', html, re.S | re.I
    ):
...
def _events_from_euskirchen(html: str) -> list:
    events = []
    for block in re.findall(r'<div class="event-list-wrapper">(.*?)</div>\s*</div>\s*</div>', html, re.S | re.I):
