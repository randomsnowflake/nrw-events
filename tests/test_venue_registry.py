...
class VenueRegistryTests(unittest.TestCase):
    def test_registry_ids_and_aliases_are_unique(self):
        ...
        aliases = [alias.casefold() for record in VENUE_REGISTRY for alias in (record.display_name, *record.aliases)]

        self.assertEqual(len(ids), len(set(ids)))
        self.assertEqual(len(aliases), len(set(aliases)))

    def test_verified_map_locations_are_unique_per_city_and_name(self):
        keys = [(record.city.casefold(), record.venue.casefold()) for record in VERIFIED_VENUE_LOCATIONS]
    ...
    def test_city_placeholder_is_removed_when_an_address_follows(self):
        resolved = resolve_venue("Bad Honnef, Quellenstraße 2, 53604 Bad Honnef", "Bad Honnef")

        self.assertEqual(resolved.venue, "")
        self.assertEqual(resolved.venue_address, "Quellenstraße 2, 53604 Bad Honnef")

    def test_postcode_and_trailing_city_are_coalesced(self):
        resolved = resolve_venue("Beethovenhalle Bonn, Wachsbleiche 16, Bonn, 53111", "Bonn")
