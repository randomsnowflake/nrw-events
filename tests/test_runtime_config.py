...
class RuntimeConfigTests(unittest.TestCase):
    def test_http_response_size_limit_is_large_but_bounded_by_default(self):
    ...
    def test_http_response_size_limit_can_be_enabled_explicitly(self):
        with mock.patch.dict(
            os.environ,
            {"NRW_EVENTS_HTTP_MAX_RESPONSE_BYTES": "20000000"},
            clear=True,
        ):
            self.assertEqual(
                config.runtime_config().http_max_response_bytes,
    ...
    def test_http_response_size_limit_can_be_disabled_explicitly(self):
        with mock.patch.dict(
            os.environ,
            {"NRW_EVENTS_HTTP_MAX_RESPONSE_BYTES": "0"},
            clear=True,
        ):
