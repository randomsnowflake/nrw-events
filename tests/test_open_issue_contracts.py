...
class OpenIssueContractTests(unittest.TestCase):
    def test_snapshot_exports_editorial_features_without_changing_score(self):
        ...
        context = RunContext(
            ...
            "features",
            configure_logging("features", "ERROR", "", ""),
    ...
    def test_unresolved_locations_are_published_and_radius_drops_are_counted(self):
        ...
        context = RunContext(
            ...
            EventWindow(datetime(2026, 8, 1), datetime(2026, 8, 28)),
            "radius",
            configure_logging("radius", "ERROR", "", ""),
        )
        unresolved = raw_event(
            "Unklarer Ort", distance_km=None, location_confidence="unresolved", venue="", venue_id=""
        )
    ...
    def test_authoritative_cancellation_keeps_occurrence_identity_and_score_floor(self):
        ...
        context = RunContext(
            ...
            EventWindow(datetime(2026, 8, 1), datetime(2026, 8, 28)),
            "cancellation",
            configure_logging("cancellation", "ERROR", "", ""),
            clock=lambda: datetime(2026, 8, 1, 12),
        )
        scheduled = raw_event()
        ...
        result = runner.run_import(
            context,
            {"Bonn.de Events": lambda: [scheduled, cancelled]},
        )
    ...
    def test_side_channel_cancellation_is_canonicalized_before_cross_run_fields(self):
        cancellation = raw_event(
            ...
            category_label="Workshops & Kurse",
        ...
        old = {
            ...
            "series": {
                "old-series": {
                    ...
                    "title": "Befristete Lesereihe",
                    "venue": "Haus",
                    "canonical_venue_id": "haus",
                    "city": "Bonn",
                    "category_key": "talk",
                    "first_seen": "2024-01-01T00:00:00",
                    "last_seen": "2024-02-01T00:00:00",
                    "occurrences": {"a": "2024-01-07", "b": "2024-02-04"},
                    "announced_dates": [],
                }
            },
        }
    ...
    def test_declarative_adapters_cover_multi_endpoint_html_and_cached_details(self):
        ical = SourceSpec(
            "multi",
            "Multi",
            ("https://example.test/a.ics", "https://example.test/b.ics"),
            AdapterType.ICAL,
            "Bonn",
        )
        with mock.patch(
            "nrw_events.source_specs.common.fetch_ical", side_effect=[[{"title": "A"}], [{"title": "B"}]]
        ) as fetch:
        ...
        with mock.patch(
            "nrw_events.source_specs.common.fetch_url",
            return_value=("<article><h2>Sommerfest</h2><time>15.08.2026</time></article>"),
        ):
        ...
        with (
            mock.patch("nrw_events.source_specs.common.fetch_url", return_value="{}"),
